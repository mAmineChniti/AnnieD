...
int main()
{
    ...
    while(state)
    {
        ...
        if(state==1)
        {
            ...
            if(check_mouse_collision(play)||key==0){display_img(screen,play_s);key=0;}
            else display_img(screen,play);
            if(check_mouse_collision(settings)||key==1){display_img(screen,settings_s);key=1;}
            else display_img(screen,settings);
            if(check_mouse_collision(quit)||key==2){display_img(screen,quit_s);key=2;}
            else display_img(screen,quit);
            while(SDL_PollEvent(&event))
            {
                ...
                {
                    if(event.key.keysym.sym==SDLK_DOWN)key=(key+4)%3;
                    else if(event.key.keysym.sym==SDLK_UP)key=(key+2)%3;
                    else if(event.key.keysym.sym==SDLK_RETURN)
                    {
                        if(key==0)state=2;
                        else if(key==1)state=3;
                        else state=0;
                    }
                }
            }
